...
const DeckOfCards: React.FC = () => {
  ...
  const [pokeArray, setPokeArray] = useState<NameURLInterface[] | null>(
    pokemonList
  );
  ...
  const handleUndo = () => {
    setExitX(0);
    pokemonIndex > 0
      ? dispatch(setPokemonIndex(pokemonIndex - 1))
      : dispatch(setPokemonIndex(0));
  };

  useEffect(() => {
    if (pokemonList) {
      const filteredResult = pokemonList.filter((pokemon) =>
            ...
            && (
              ...
              >
                {pokeArray && cardIndex + 2 < pokemonListLength ? (
                  <CardWrapper
                    id={+getIDStringfromURL(pokeArray[cardIndex + 2].url)}
                  />
                ) : (
                  <EndCard />
                )}
            ...
            {pokemonListLength >= 2 && cardIndex + 1 < pokemonListLength + 1 && (
              ...
              >
                {pokeArray && cardIndex + 1 < pokemonListLength ? (
                  <CardWrapper
                    id={+getIDStringfromURL(pokeArray[cardIndex + 1].url)}
                  />
                ) : (
                  <EndCard />
                )}
            ...
            {pokemonListLength >= 1 && cardIndex < pokemonListLength + 1 && (
              ...
              >
                {pokeArray && cardIndex < pokemonListLength ? (
                  <CardWrapper
                    id={+getIDStringfromURL(pokeArray[cardIndex].url)}
                  />
                ) : (
                  <EndCard />
                )}
