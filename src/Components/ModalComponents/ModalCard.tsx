...
const ModalCard: React.FC = () => {
  ...
  const { data: dataP } = useQuery(['fetchPokemonData', modalIndex], async () =>
    fetchPokemonData(modalIndex)
  )
  const { data: dataS } = useQuery(['fetchPokemonSpeciesData', modalIndex], async () =>
  ...
  return (
    ...
    >
      ...
      <div className="h-max w-full">
        {dataP && dataS ? <LeftCard speciesdata={dataS} pokemondata={dataP} /> : <LeftCardLoader />}
      </div>
      <div className="h-max w-full">
        {dataP && dataS ? (
